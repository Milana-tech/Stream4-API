...
public class ReferralController
{
    ...
    @Operation (summary = "Apply referral discount", description = "Apply referral discount to both inviter and " +
            "invitee. Supports JSON, XML, CSV.")
    public ResponseEntity<ReferralDiscountResponse> applyReferralDiscount(@PathVariable String inviteeId)
    {
        ...
        {
            return ResponseEntity.badRequest().body(new ReferralDiscountResponse(null, invitee.getUserId(), false,
                    false, "Invitee was not invited by anyone."));
        }

        User inviter = userRepository.findById(invitee.getInvitedBy()).orElseThrow(() -> new RuntimeException(
                "Inviter not found."));

        if (invitee.isReferralDiscountUsed() || inviter.isReferralDiscountUsed())
        {
            return ResponseEntity.badRequest().body(new ReferralDiscountResponse(inviter.getUserId(),
                    invitee.getUserId(), false, false, "Discount is already used by one of the accounts"));
        }

        Subscription inviteeSub = subscriptionRepository.findByUser_UserIdAndStatus(inviteeId,
                SubscriptionStatus.ACTIVE).orElseThrow(() -> new RuntimeException("Invitee subscription not found."));

        Subscription inviterSub = subscriptionRepository.findByUser_UserIdAndStatus(inviter.getUserId(),
                SubscriptionStatus.ACTIVE).orElseThrow(() -> new RuntimeException("Inviter subscription not found"));
        ...
        return ResponseEntity.ok(new ReferralDiscountResponse(inviter.getUserId(), invitee.getUserId(), true, true,
